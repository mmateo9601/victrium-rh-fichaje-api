...
| `DB_HOST` | Conditional | No | MySQL host when not using `DATABASE_URL` | `db.example.com` | Runtime |
| `DB_PORT` | Conditional | No | MySQL port | `3306` | Runtime |
| `DB_NAME` | Conditional | No | MySQL database name | `victrium_rh` | Runtime |
| `DB_USER` | Conditional | Yes | MySQL username | `victrium_user` | Runtime |
| `DB_PASSWORD` | Conditional | Yes | MySQL password | `replace-with-strong-password` | Runtime |
...
| `SMTP_HOST` | No | Yes | SMTP server host | `smtp.example.com` | Runtime |
| `SMTP_PORT` | No | Yes | SMTP server port | `587` | Runtime |
| `SMTP_USER` | No | Yes | SMTP username | `mailbox@example.com` | Runtime |
| `SMTP_PASSWORD` | No | Yes | SMTP password | `replace-with-strong-password` | Runtime |
