...
- El dominio de turnos vive en `/api/v1/shifts`, `/api/v1/shift-assignments`, `/api/v1/planning-periods` y `/api/v1/schedule`, con vista propia en Next.js para planificación, calendario personal y detalle de turno
- Los centros de trabajo viven en `/api/v1/work-locations` y `/api/v1/employee-location-assignments`, y permiten histórico de movilidad por empleado
- Las integraciones servidor-a-servidor usan `x-api-key` y se administran desde `/api/v1/api-keys`

## Estado

La base de auth, fichajes, `companies/users/employees`, `work locations`, `planning periods`, `vacations`, `incidents`, `calendars`, `api keys`, `profile` y `turnos` ya están conectadas.
