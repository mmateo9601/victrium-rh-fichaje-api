...
| Work locations | Multi-site centers with calendar support | Multi-site model with calendar and schedule support | Work locations are implemented with calendar linkage and UI | P0 | High | Extend with richer hierarchy and provisioning flows | IMPLEMENTED |
...
| Scheduling | Shift templates, assignments, overrides, schedule views | Rotations, segments, publishable planning periods, multi-location planning | Planning periods, split segments and rotation patterns are now supported | P1 | High | Expand rotation editor and assignment generation flows | PARTIAL |
